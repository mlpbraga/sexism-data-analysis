...
class Comments:
    ...
    def _set_vocabularies(self):
        self.vocabulary, self.freq, self.word_list = get_vocabulary(self.labeled_comments)
    ...
    def _set_word_frequencies(self):
        ...
        for word in shared_words:
            ...
            if word in self.sexist_vocabulary.keys():
                word_freq['not-sexist-freq'].append(self.not_sexist_vocabulary[word])
            else:
                word_freq['not-sexist-freq'].append(0)
            if word in self.undefined_vocabulary.keys():
                word_freq['undefined-freq'].append(self.undefined_vocabulary[word])
            else:
                word_freq['undefined-freq'].append(0)
            word_freq['diff'] = self.sexist_vocabulary[word] - self.not_sexist_vocabulary[word]
    ...
    def _set_tf(self):
        ...
        self.tf_sexist_dataframe = pd.concat([sexist_tf, not_sexist_tf]).fillna(0)
        ...
        not_sexist_tf = pd.DataFrame(
            not_sexist_vectorizer.fit_transform(not_sexist_doc).toarray())
        self.tf_not_sexist_dataframe = pd.concat([sexist_tf, not_sexist_tf]).fillna(0)
    ...
    def _set_feature_dataframe(self):
        try:
            ...
            tf_dataframe = pd.concat(
                ...
                self.tf_not_sexist_dataframe], axis=1)
